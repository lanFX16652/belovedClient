...
export const postAddItem = createAsyncThunk("/cart/add", async (cartData) => {
    const { productId, qty } = cartData;
    try {
        const response = await axiosInstance.post("/add-to-cart", { productId, qty })
...
export const cartSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(postAddItem.pending, (state, action) => { state.isLoading = true })
